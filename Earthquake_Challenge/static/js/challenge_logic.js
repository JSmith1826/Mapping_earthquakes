...
d3.json(majorearth).then(function(data) {
  ...
  // 5 - Change the color function from asynce to use three colors for the major earthquakes based on the magnitude of the earthquake.
  function getColor(magnitude) {
    if (magnitude > 5) {
      return "#ea2c2c";
    }
  ...
  // 6 - Use the function from the async unit that determines the radius of the earthquake marker based on its magnitude.
  function getRadius(magnitude) {
    if (magnitude === 0) {
      return 1;
    }
  ...
  // 7 - Create a GeoJSON layer with the retrieved data that adds circleMarkers to the map, sets the style 
  // and adds the popups.
  L.geoJson(data, {
    pointToLayer: function(feature, latlng) {
    return L.circleMarker(latlng);
